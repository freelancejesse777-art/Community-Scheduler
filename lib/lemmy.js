...
async function getPostStats(instanceUrl, jwt, postId) {
  const url = `${instanceUrl.replace(/\/$/, "")}/api/v3/post?id=${postId}`;
  const res = await fetch(url, {
    headers: { Authorization: `Bearer ${jwt}` },
  });
  if (!res.ok) {
    throw new Error(`Lemmy post lookup failed (${res.status}): ${await res.text()}`);
  }
  const data = await res.json();
  ...
  return {
    ...
    url: `${instanceUrl.replace(/\/$/, "")}/post/${postId}`,
  };
}

module.exports = { lemmyLogin, postToLemmy, getPostStats };
