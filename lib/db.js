...
// Helper used by every migration below — safe to run from multiple
// concurrent processes (Next.js's build step evaluates many route files
// in parallel workers, so more than one can race to add the same column).
// If we lose the race, SQLite reports "duplicate column name" — that just
// means another worker already added it, which is fine to ignore.
function safeAddColumn(table, name, type) {
  try {
