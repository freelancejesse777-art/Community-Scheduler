...
async function createCheckoutSession(user, successUrl, cancelUrl) {
  ...
  if (customerId) {
    // A customer ID saved while using a test-mode key won't exist once
    // you switch to a live-mode key (or vice versa) — test and live are
    // completely separate universes of Stripe objects that happen to
    // share the same ID format. Verify the stored ID is actually usable
    // under the CURRENT key before trying to reuse it; if not, treat it
    // as if there were none and create a fresh one below. This makes
    // test->live (and any other) mode switches self-healing instead of
    // leaving a stale, permanently-broken customer id sitting in the DB.
    try {
